...
public class Stage_A_Manager : MonoBehaviour
{
    ...
    GameObject[] Arrow_Img_Instance_GO; // 장애물 인스터스 배열 선언.
    ...
    void Start()
    {
        System.Array.Resize(ref Arrow_Img_Instance_GO, (arrow_Max_Count)); // 배열크기 arrow_Max_Count 만큼 늘리기.
    ...
    {
        Cheez_Img_GO.transform.Translate(20f, 0, 0);
    ...
    void Update()
    {
        ...
        if (time >= spawn_time)
        {
            ...
            for (int i = 0; i < arrow_Max_Count; i++)
            {
                if (Arrow_Img_Instance_GO[i].activeSelf == false) //비활성화 오브젝트 발견시.
                {
                    go_index = i; //인덱스값 전달.
        ...
        {
            Debug.Log("골인!");
            Scene_Manager.Instance.Game_Stage_B(); // 스테이지 B 로 이동.
